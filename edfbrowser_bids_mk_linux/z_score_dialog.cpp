...
*
* Copyright (C) 2012 - 2025 Teunis van Beelen
*
...
void UI_ZScoreWindow::addTraceButtonClicked()
{
  ...
  signalcomp->zratio_filter = create_zratio_filter(hdr->edfparam[signalcomp->edfsignal[0]].smp_per_record,
                                                   ...
                                                   signalcomp->edfparam_0->bitvalue);

  if(signalcomp->zratio_filter == NULL)
  {
    QMessageBox messagewindow(QMessageBox::Critical, "Error", "Cannot create a Z-ratio-filter, malloc() error?");
    messagewindow.exec();
    free(signalcomp);
    return;
  }
  ...
  if(dblcmp(signalcomp->edfparam_0->bitvalue, 0.0) < 0)
  {
    signalcomp->sensitivity = signalcomp->edfparam_0->bitvalue / -1.0 / mainwindow->y_pixelsizefactor;
...
//   mainwindow->z_score_var.zscore_error_detection = zscore_error_detection;
  ...
  dftblocksize = ((long long)smp_per_record * (ZSCORE_EPOCH_LEN * TIME_FIXP_SCALING)) / hdr->long_data_record_duration;
  ...
  samplefreq = (double)smp_per_record / ((double)hdr->long_data_record_duration / TIME_FIXP_SCALING);
...
//        epochs / zscore_page_len);
  ...
  flt_blck_rd_t *blockread = create_flt_blck_rd(signalcomp, 1, 0, 0, NULL, &buf);
  if(blockread == NULL)
  {
...
// printf("smpls_in_inputbuf is %i\n", smpls_in_inputbuf);
    ...
    if(run_flt_blck_rd(blockread, datrec_offset))
    {
      progress.reset();
    ...
    smpls = get_samples_flt_blck_rd(blockread);
...
void UI_ZScoreWindow::get_annotationsButtonClicked()
{
  ...
  edfhdrblck_t *edf_hdr = mainwindow->signalcomp[signalnr]->edfhdr;
  ...
  for(i=0; i<marker_end; i++)
  {
    if(zscore_sleepstage_buf[i] != awake)
    {
      ...
      if(i >= marker_start)
      {
        ...
        annotation.onset = i * zscore_page_len * 2LL * TIME_FIXP_SCALING;
        if(awake)
        {
...
void UI_ZScoreWindow::jumpButtonClicked()
{
  ...
  if(plot_type)
  {
    onset = position * zscore_page_len * 2 * TIME_FIXP_SCALING;
  }
  else
  {
    onset = position * ZSCORE_EPOCH_LEN * TIME_FIXP_SCALING;
  }

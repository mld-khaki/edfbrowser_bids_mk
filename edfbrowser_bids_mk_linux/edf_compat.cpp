...
#include <cstring>
    ...
    // Your function uses: char summary[16384];
    ...
    // Provides the 2-arg strcat_s(dst, src) your code calls.
    // It clamps to the summary buffer capacity to prevent overflow.
    static inline void strcat_s(char *dst, const char *src)
    {
        if (!dst || !src) return;

        constexpr size_t CAP = 16384;  // matches: char summary[16384];
        size_t cur = strnlen_compat(dst, CAP);
        if (cur >= CAP - 1) return;

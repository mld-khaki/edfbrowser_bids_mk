...
*
* Copyright (C) 2011 - 2025 Teunis van Beelen
*
...
UI_AverageCurveWindow::UI_AverageCurveWindow(sigcompblck_t *signal_comp, QWidget *w_parent, int number,
                                             double *abuf,
                                             double maxvalue,
                                             double minvalue,
                                             long long apagetime,
                                             ...
                                             int avg__period)
{
  ...
  strlcpy(str1_1024, "Averaging  ", 1024);
  strlcat(str1_1024, signalcomp->signallabel, 1024);
  averager_curve_dialog->setWindowTitle(str1_1024);
  ...
  snprintf(str1_1024, 1024, "Averaging %.32s  %i triggers \"%s\"", signalcomp->signallabel, avg_cnt, avg_annotation);
  curve1->setUpperLabel1(str1_1024);
...
void UI_AverageCurveWindow::export_edf(void)
{
  ...
  smp_per_record = signalcomp->edfparam_0->smp_per_record;
  ...
  get_filename_from_path(path + strlen(path), signalcomp->edfhdr->filename, MAX_PATH_LENGTH - strlen(path));
  ...
  for(j=0; j<signalcomp->fir_filter_cnt; j++)
  {
    p += snprintf(str + p, 4096 - p, "FIR ");
  }

  strlcat(str, signalcomp->edfparam_0->prefilter, 4096);
  edf_set_prefilter(edf_hdl, 0, str);
  edf_set_transducer(edf_hdl, 0, signalcomp->edfparam_0->transducer);
  ...
  if((signalcomp->edfhdr->edfplus) || (signalcomp->edfhdr->bdfplus))
  {
    ...
    if(signalcomp->edfhdr->plus_sex[0] == 'M')
    {
      edf_set_sex(edf_hdl, 1);
    }
    if(signalcomp->edfhdr->plus_sex[0] == 'F')
    {
      edf_set_sex(edf_hdl, 0);
    }
  ...
  edfwrite_annotation_latin1_hr(edf_hdl,
                               (avg_period * 1000) / avg_trigger_position_ratio,
                               -1,
                               avg_annotation);
